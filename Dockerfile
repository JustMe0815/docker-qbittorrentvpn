...
RUN apt-get update && apt-get install -y --no-install-recommends \
    g++ \
    libxml2-utils \
    && BOOST_VERSION_DOT=$(curl -s "https://www.boost.org/feed/news.rss" | \
    xmllint --xpath '//rss/channel/item/title/text()' - | \
    awk -F 'Version' '{print $2 FS}' - | sed -e 's/Version//g;s/\ //g' | xargs | awk 'NR==1{print $1}' -) \
    ...
    && cd /opt && rm -rf /opt/* \
    && apt-get purge -y g++ libxml2-utils && apt-get clean && rm -rf /var/lib/apt/lists/*

# Install Ninja
RUN apt-get update && apt-get install -y --no-install-recommends \
    jq \
    unzip \
    && NINJA_ASSETS=$(curl -s "https://api.github.com/repos/ninja-build/ninja/releases" | \
    jq '.[] | select(.prerelease==false) | .assets_url' | head -n 1 | tr -d '"') \
    && NINJA_DOWNLOAD_URL=$(curl -s "${NINJA_ASSETS}" | \
    ...
    && mv /opt/ninja /usr/local/bin/ninja && chmod +x /usr/local/bin/ninja \
    && rm -rf /opt/* && apt-get purge -y jq unzip && apt-get clean && rm -rf /var/lib/apt/lists/*

# Install CMake
RUN apt-get update && apt-get install -y --no-install-recommends \
    jq \
    && CMAKE_ASSETS=$(curl -s "https://api.github.com/repos/Kitware/CMake/releases" | \
    jq '.[] | select(.prerelease==false) | .assets_url' | head -n 1 | tr -d '"') \
    && CMAKE_DOWNLOAD_URL=$(curl -s "${CMAKE_ASSETS}" | \
    ...
    && rm -rf /opt/* && apt-get purge -y jq && apt-get clean && rm -rf /var/lib/apt/lists/*

# Compile and install libtorrent-rasterbar
RUN apt-get update && apt-get install -y --no-install-recommends \
    build-essential \
    libssl-dev \
    && LIBTORRENT_ASSETS=$(curl -s "https://api.github.com/repos/arvidn/libtorrent/releases" | \
    jq '.[] | select(.prerelease==false) | select(.target_commitish=="RC_1_2") | .assets_url' | head -n 1 | tr -d '"') \
    && LIBTORRENT_DOWNLOAD_URL=$(curl -s "${LIBTORRENT_ASSETS}" | jq '.[0] .browser_download_url' | tr -d '"') \
    && LIBTORRENT_NAME=$(curl -s "${LIBTORRENT_ASSETS}" | jq '.[0] .name' | tr -d '"') \
    ...
    && cmake --install build \
    && cd /opt && rm -rf /opt/* && apt-get purge -y build-essential libssl-dev && apt-get clean && rm -rf /var/lib/apt/lists/*
...
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    qttools5-dev \
    zlib1g-dev \
    && QBITTORRENT_RELEASE=$(curl -s "https://api.github.com/repos/qBittorrent/qBittorrent/tags" | \
    jq '.[] | select(.name | index ("alpha") | not) | select(.name | index ("beta") | not) | select(.name | index ("rc") | not) | .name' | head -n 1 | tr -d '"') \
    ...
    && cmake --build build --parallel $(nproc) \
    && cmake --install build \
    && cd /opt && rm -rf /opt/* && apt-get purge -y build-essential git libssl-dev pkg-config qtbase5-dev qttools5-dev zlib1g-dev && apt-get clean && rm -rf /var/lib/apt/lists/*

# Install WireGuard and additional dependencies
...
# Add OpenVPN and qBittorrent configuration files
